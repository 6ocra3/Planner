                ...
                <ChevronLeft onClick={() => {
                    const fetchWeekTasks = () => {
                        return async (dispatch) => {
                            ...
                            const responseTasks = await fetch(`http://127.0.0.1:5005/get_week_tasks/${newMondayDateF.slice(0, 10)}`);
                            const tasks = await responseTasks.json();

                            const responseWeek = await fetch(`http://127.0.0.1:5005/get_week/${newMondayDateF.slice(0, 10)}`);
