...
import {
  dragDropTrackerTask,
  dragDropTrackerTasks,
  dragEndTrackerTask,
  dragOverTrackerTask,
  dragStartTrackerTask,
  ...
  return (
    ...
    >
          ...
          return (
            <div
              ...
              className={
                "task__day " +
                cs[v] +
                (tasks[keyId].status != 0 ? " task_finished" : "")
              }
            >
              {v == 3 && <ArrowRight size={18}></ArrowRight>}
            </div>
          );
        })}
      </div>
      <div
        ...
        onDrop={(e) =>
          dragDropTrackerTask(e, keyId, index, trackerOrder, dispatch)
        }
      >
        <div className="top_line"></div>
        <h4
          className={
            "task__text " + (tasks[keyId].status != 0 ? "task_finished" : "")
          }
          onDragStart={(e) => dragStartTrackerTask(e, keyId)}
          draggable
        >
          {task.length * 14 > 150 ? task.slice(0, 12) + "..." : task}
        </h4>
